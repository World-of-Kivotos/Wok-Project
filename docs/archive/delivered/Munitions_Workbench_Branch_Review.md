...
> ## 归档件 — 历史记录, 不是实现依据
>
> - **冻结日期**: 2026-07-12 (全部立案项闭环, 分支已合并)
> - **冻结基线**: 审查基线 main `73a22a4`
> - **归档日期**: 2026-09-20 (归档基线 `bd0c4588`)
> - **现行真源**: `docs/Munitions_Job_DesignSpec.md`
>
> 本文自冻结之日起**不再回写**。其中的结论、行号、状态标记反映的是冻结时点的事实,
> 后续提交已推翻其中相当一部分。**不要把本文当作现状依据做决策**, 也不要按本文的"处置建议"直接动手。
>
> **注意本文自相矛盾**: 第二节的 21 个立案项勾选框全部是未勾状态, 而文末"修复记录 (2026-07-12)"
> 一节写着"全部立案项闭环, 分支达合并态"。以后者为准 —— 勾选框是审查当时留下的, 修完没有回勾。
> 头部"结论: 不可合并"同样是审查当时的结论, 早已作废。
...
# 修复记录 (2026-07-12): 全部立案项闭环, 分支达合并态

与作者对齐后的三项决策: 生产模型取**双模式共存** (空闲挂机被动结算 + 开工期手动/连续制作, 天然互斥);
料槽保留**四件套并补获取链** (成本精确对齐文档 7 铜+16 火药); **gunsmith 整体 config 禁用**待完善分支。

按原子提交逐项落地 (每步 GameTest 全绿):

| 立案 | 处置 | 提交 |
| --- | --- | --- |
| C-1 爆炸双掉落 dupe | 六份 loot 加 part=main 条件 + 生存路径删主动清除交级联 + 创造挖 extension 抑制掉落 + 放置补边界校验; 活塞推 extension 的丢台面顺带闭合 | fix(munitions) C-1 |
| M-1 被动产线死代码 | settleManualCraft 空闲返 false, 离线补产复活, 2 红回绿; 清批推进 lastSettleTick 防双重产出 | fix(munitions) M-1/M-2 |
| M-2 开工扣料不退 | 扣料挪到完成帧与工费同帧原子结算, 取消/中断零损失, 连续模式失败停机 | 同上 |
| M-3 归属错位 | 开工/取消/连续开关全部限台主 (isOwner), 产量/工费/经验同源 | fix(munitions) M-3 |
| M-4 四件套无获取链 | 合成表 底火=2铜(横)/弹头=2铜(竖)/弹壳=3铜/发射药=8火药, 每批 1/1/1/2 = 7 铜+16 火药 -> 40 发 (L6+ 70); 测试备料按 config cost 缩放 | feat(munitions) M-4 |
| M-5 存量降档 | 旧注册名恢复全档 (1,10), lang 复名 "军火台", 新档位走新注册名 | fix(munitions) M-5 |
| M-6 WebUI 越界重写 | 整文件回退 main 实测版 (d99e19e) | fix(webui) M-6 |
| M-7 Shift 并入输出槽销毁 | quickMoveStack 玩家侧目标区间止步输出槽 | fix(munitions) M-7/M-8 |
| M-8 并发取弹超扣 | 取弹计量改 Slot.remove 精确累计 + Shift 差值结算, 替换 getItem 快照; 超扣先取证日志再钳 | 同上 |
| M-9 进度 int16 回绕 | ContainerData 改按秒过线, Menu 侧 x20 还原 | fix(munitions) M-9 |
| M-10 i18n 回退 | 9 处中文字面量 + 口径类别标签全部 translatable 化, 补双语 lang | fix(munitions) M-10 |
| M-11 测试盲区 | 补 5 例: 手动制作原子结算/费不足保料停机/非主人全拒/档位钳制+旧台全档回归/输出槽合并强断言 | test(munitions) M-11 |
| C-2/G-1~G-4 (gunsmith) | GUNSMITH_ENABLED 默认 false, 门控冲压台交互/M4 组枪/TACZ 加伤事件/创造页签, 摘模板配方; 待完善分支补材料链与战力评审后启用 | fix(munitions) 功能门 |
| minor 系列 | 自绘按钮只认左键 (对话框全键位模态)/死方法删除/yieldFactor 不可达兜底删除/取消文案同步零损失语义/创造掉落/边界校验 | 并入各提交 |
| F-1 farmer 耕地 | 行为保留 (有测试背书); 存量世界耕地上支撑物弹出属一次性迁移扰动, 上线公告即可 | 不改代码 |
| 未处置 (低危遗留) | UI 搜索面板与输出槽几何重叠 5-8px (美术布局, 留作者调); GunsmithPressScreen 硬编码中文 (随子系统禁用, 完善分支一并) | 记录在案 |

验证: 修复期间分支 426->431 全绿; merge origin/main (解 lang 双侧追加冲突) 后 **652 required GameTest 全绿**, 零回归。分支达合并态。
