...
PARTS: dict[tuple[int, int], list[dict]] = {
    (block_x, block_y): [] for block_x in range(3) for block_y in range(2)
}
...
def add_local(
    ...
    if (block_x, block_y) not in PARTS:
        raise ValueError(f"Unknown part for {name}: {(block_x, block_y)}")
    ...
    PARTS[(block_x, block_y)].append(element)
...
def add_global_box(
    ...
    if end[0] > 48 or end[1] > 32 or end[2] > 16:
        raise ValueError(f"Global coordinates exceed 48x16x32 for {name}")
    for block_x in range(3):
        ...
        for block_y in range(2):
            ...
            add_local(
                block_x,
                block_y,
                f"{name}_x{block_x}_y{block_y}",
                (x0 - block_x * 16, y0 - block_y * 16, start[2]),
                (x1 - block_x * 16, y1 - block_y * 16, end[2]),
                material,
                shade=shade,
            )
...
def build_geometry() -> None:
    # Full-span underframe and skids establish the exact 48x16 footprint.
    add_global_box("front_floor_rail", (0, 0, 0), (48, 1.25, 1.5), "frame")
    add_global_box("rear_floor_rail", (0, 0, 14.5), (48, 1.25, 16), "frame")
    add_global_box("left_floor_rail", (0, 0, 1.5), (1.5, 1.25, 14.5), "frame")
    add_global_box("right_floor_rail", (46.5, 0, 1.5), (48, 1.25, 14.5), "frame")
    add_global_box("front_skid", (1.25, 1.25, 0.75), (46.75, 2.25, 2.25), "frame")
    add_global_box("rear_skid", (1.25, 1.25, 13.75), (46.75, 2.25, 15.25), "frame")

    # Corner and bay-divider frame posts remain visible through both tiers.
    for index, (x0, x1) in enumerate(((0.25, 1.75), (15.25, 16.75), (31.25, 33.0), (46.25, 47.75))):
        ...
        add_global_box(f"rear_post_{index}", (x0, 2.25, 14.3), (x1, 27.25, 15.75), "frame")
        add_global_box(f"front_foot_{index}", (max(0, x0 - 0.35), 0, 0), (min(48, x1 + 0.35), 4, 2.5), "frame")
        add_global_box(f"rear_foot_{index}", (max(0, x0 - 0.35), 0, 13.5), (min(48, x1 + 0.35), 4, 16), "frame")
        add_global_box(f"front_cap_{index}", (max(0, x0 - 0.35), 25.5, 0), (min(48, x1 + 0.35), 28.25, 2.5), "frame")
        add_global_box(f"rear_cap_{index}", (max(0, x0 - 0.35), 25.5, 13.5), (min(48, x1 + 0.35), 28.25, 16), "frame")

    add_global_box("front_top_rail", (1.4, 26.25, 0.5), (46.6, 27.5, 1.55), "frame")
    add_global_box("rear_top_rail", (1.4, 26.25, 14.45), (46.6, 27.5, 15.5), "frame")
    add_global_box("left_top_rail", (0.5, 26.25, 1.5), (1.55, 27.5, 14.5), "frame")
    add_global_box("right_top_rail", (46.45, 26.25, 1.5), (47.5, 27.5, 14.5), "frame")
    ...
    add_rotated(0, 0, "left_door_lower_left_bevel", (2.65, 4.7, 0.3), (3.75, 8.1, 1.25), "panel_light", "z", 45, (4.0, 6.4, 0.8))
    add_rotated(0, 0, "left_door_lower_right_bevel", (13.25, 4.7, 0.3), (14.35, 8.1, 1.25), "panel_light", "z", -45, (13.0, 6.4, 0.8))
    add_rotated(0, 1, "left_door_upper_left_bevel", (2.65, 0.1, 0.3), (3.75, 3.5, 1.25), "panel_light", "z", -45, (4.0, 1.8, 0.8))
    add_rotated(0, 1, "left_door_upper_right_bevel", (13.25, 0.1, 0.3), (14.35, 3.5, 1.25), "panel_light", "z", 45, (13.0, 1.8, 0.8))
    ...
    add_rotated(0, 1, "left_roof_front_slope", (3.0, 4.0, 1.2), (14.4, 5.15, 5.2), "panel_light", "x", -22.5, (8.7, 4.6, 3.2))
    ...
    add_rotated(1, 0, "center_octagon_lower_left", (1.05, 5.4, 0), (2.35, 10.0, 1.55), "panel_light", "z", 45, (3.0, 7.7, 0.8))
    add_rotated(1, 0, "center_octagon_lower_right", (14.4, 5.4, 0), (15.7, 10.0, 1.55), "panel_light", "z", -45, (13.75, 7.7, 0.8))
    add_rotated(1, 1, "center_octagon_upper_left", (1.05, 0.0, 0), (2.35, 4.6, 1.55), "panel_light", "z", -45, (3.0, 2.3, 0.8))
    add_rotated(1, 1, "center_octagon_upper_right", (14.4, 0.0, 0), (15.7, 4.6, 1.55), "panel_light", "z", 45, (13.75, 2.3, 0.8))
    ...
    add_rotated(1, 0, "center_lower_left_fold", (1.0, 3.0, 0.55), (2.1, 6.0, 1.8), "panel", "z", 45, (2.2, 4.5, 1.0))
    add_rotated(1, 0, "center_lower_right_fold", (14.7, 3.0, 0.55), (15.8, 6.0, 1.8), "panel", "z", -45, (14.6, 4.5, 1.0))

    add_global_box("center_upper_brow", (16.7, 20.5, 0.75), (32.3, 22.55, 3.2), "panel")
    add_rotated(1, 1, "center_brow_slope", (1.1, 5.05, 0.9), (15.8, 6.35, 5.5), "panel_light", "x", -22.5, (8.45, 5.7, 3.2))
    add_global_box("center_roof_middle", (17.8, 22.4, 3.1), (32, 25.35, 13.35), "panel")
    add_rotated(1, 1, "center_roof_fold", (2.65, 8.35, 2.5), (15.5, 10.0, 13.5), "panel_light", "x", -22.5, (9.1, 9.2, 8.0))
    ...
    add_rotated(2, 0, "right_lower_left_bevel", (1.15, 4.1, 0.45), (2.3, 7.4, 1.65), "panel_light", "z", 45, (2.5, 5.75, 1.0))
    add_rotated(2, 0, "right_lower_right_bevel", (13.25, 4.1, 0.45), (14.4, 7.4, 1.65), "panel_light", "z", -45, (13.05, 5.75, 1.0))
    add_rotated(2, 1, "right_upper_left_bevel", (1.15, 0.9, 0.45), (2.3, 4.2, 1.65), "panel_light", "z", -45, (2.5, 2.55, 1.0))
    add_rotated(2, 1, "right_upper_right_bevel", (13.25, 0.9, 0.45), (14.4, 4.2, 1.65), "panel_light", "z", 45, (13.05, 2.55, 1.0))

    add_global_box("right_roof_lower", (32.8, 20.3, 2), (46.2, 22.6, 14), "panel")
    add_rotated(2, 1, "right_roof_front_slope", (1.15, 5.25, 1.2), (14.2, 6.55, 5.3), "panel_light", "x", -22.5, (7.7, 5.9, 3.25))
    ...
    for block_x in range(3):
        for block_y in range(2):
            model = {
                ...
                "elements": [json_element(element) for element in PARTS[(block_x, block_y)]],
            }
            path = MODEL_DIR / f"part_x{block_x}_y{block_y}.json"
...
def element_vertices(block_x: int, block_y: int, element: dict) -> list[tuple[float, float, float]]:
    ...
    return [(x + block_x * 16, y + block_y * 16, z) for x, y, z in transformed]
...
def project(point: tuple[float, float, float]) -> tuple[float, float]:
    x, y, z = point
    return 105 + x * 21.0 + z * 9.0, 765 - y * 20.0 - z * 7.0 + x * 1.4


def render_preview() -> None:
    width, height = 1320, 860
    canvas = bytearray((236, 238, 240, 255) * (width * height))
    ground = [(65, 775), (1120, 842), (1285, 730), (220, 675)]
    fill_polygon(canvas, width, height, ground, (210, 214, 218, 255))

    renderables = []
    for (block_x, block_y), elements in PARTS.items():
        for element in elements:
            vertices = element_vertices(block_x, block_y, element)
...
def validate_models(model_paths: list[Path], texture_paths: list[Path]) -> dict:
    ...
    if len(model_paths) != 6:
        errors.append(f"Expected 6 models, found {len(model_paths)}")
    ...
    for path in model_paths:
        ...
        stem = path.stem
        block_x = int(stem.split("_x", 1)[1].split("_", 1)[0])
        block_y = int(stem.rsplit("_y", 1)[1])
        for source in PARTS[(block_x, block_y)]:
            transformed_vertices.extend(element_vertices(block_x, block_y, source))
