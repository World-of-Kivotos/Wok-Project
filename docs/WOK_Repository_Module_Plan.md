...
- `WOK-卡牌游戏独立MOD` 是 WOK 计划中的独立安装产品，正式 modId 为 `wok_cardgame`，源码目录为 `standalone/wok-cardgame`；不得接入 WOK 本体的 `MiningDim`、网络包或资源命名空间。
...
| --- | --- | --- | --- |
| WOK-核心模块 | `wok-core` | `core`、`config`、`network`、`registry`、`menu`、`effect`、`entry`、`error`、`testutil` | 生命周期、配置、公共网络、注册辅助、玩家公共数据、错误边界、公共接口与服务装配 |
...
- 根入口一次装配约三十个 `Subsystem`，已经具备逻辑模块雏形，但所有功能仍共享一个源码集和资源命名空间。
...
- `tmp`、`outputs`、`standalone` 在根目录形成大量未跟踪内容；其中 `standalone/wok-infantry-armor` 属于 WOK步战附属产品线，不能作为 WOK 本体模块参与本次整理。
- 当前工作分支与工作区中的酿酒师改动不一致，后续必须先按模块归拢分支和提交，再进行搬迁。
- 当前酿酒师存在未提交改动，第一批结构调整不得触碰 `job/brewer` 及对应资源。
...
3. 区分设计源文件、可复现生成脚本、发布成品和临时预览；补齐 `.gitignore`，再单独审查已跟踪的 `dist` 文件。
...
3. WOK-厨师模块。
4. WOK-塔罗师模块。
5. WOK-婚姻社交模块与 WOK-实体堆叠模块。
6. WOK-市场模块与 WOK-WebUI 模块：先固化事务与动作接口。
7. WOK-军火商模块。
8. WOK-精英怪模块与 WOK-特勤干员模块。
9. WOK-铸甲师模块：资源和模型最多，并涉及 `WOK-本体护甲` 兼容，最后迁移。
10. WOK-酿酒师模块：待当前未提交工作收口后再整理。
