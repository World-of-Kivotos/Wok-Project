...
>
> - **冻结日期**: 2026-08-05
> - **冻结基线**: 审计当日主线 (报告未记录具体 commit)
> - **归档日期**: 2026-09-20 (归档基线 `bd0c4588`)
> - **现行真源**: `docs/Economy_BalanceSheet_DesignSpec.md` (顶层收支总表)
>
> 本文自冻结之日起**不再回写**。其中的结论、行号、状态标记反映的是冻结时点的事实,
> 后续提交已推翻其中相当一部分。**不要把本文当作现状依据做决策**, 也不要按本文的"处置建议"直接动手。
>
> **本文八条 Critical 中至少四条已在代码侧闭合**: [3] 塔罗卡包量纲错配、[4] AZURE 回收率、
> [6] Champions 奖励静默归零 (`AgentChampionData.java` 已整条删除)、[7] 跳蚤市场游戏内不可达
> (已有默认 G 键入口)。头部"48% / 不能上线跑"的总评建立在这些已失效的前提上。
>
> **另有两条本文写反了**: `/farmer sell` 现已有精通等级门 (`level >= 2`);
> `IJobService.level()` 并非恒返 1, 它委派到玩家 capability 的 `JobProgress.level()`。
>
> **仍未闭合、已搬进 `docs/Economy_BalanceSheet_DesignSpec.md` 的条目**:
>
> - faucet 计数键仍是**单账号维度** —— 跨账号洗额度漏洞的根因, 未修
> - 缺口[12] 市场交付写玩家背包 NBT 仍在数据库事务之外 (钱货两清但物品没进包的窄路径)
> - `/economy grant` 与 `/mchampion summon` 两条 OP 命令全库零文档 (前者绕过全部 faucet 计数)
>
> **本文曾被代码当成长期设计依据**: `quest/QuestConfig.java`、`QuestRewards.java`、
> `QuestDefinition.java` 三处 javadoc 引本文论证任务 faucet 另开独立键。该论证的落点已改到经济总表。
