...
> - **现行真源**: `MiningServerConfig.ENTRY_FEE_*` 与 `entry.MiningWebUiActions`
>
> 本文自冻结之日起**不再回写**。其中的结论、行号、状态标记反映的是冻结时点的事实,
> 后续提交已推翻其中相当一部分。**不要把本文当作现状依据做决策**, 也不要按本文的"处置建议"直接动手。
> 头部"数值待实测"的状态部分仍成立: 机制已落地, 但 `entryFeeEasy/Medium/Hard`
> 三档默认值至今都是 0 (等于功能在、没开价), 配置注释里写着 "values await yield measurement before tuning"。
