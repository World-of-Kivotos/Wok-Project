...
>
> **索引表里"阻塞状态"一列全部已过期** —— 四份规格均已实施并合入 main:
>
> | 规格 | 现行实现 |
> |---|---|
> | 矿洞三难度死亡规则 | `com.miningdim.rules.MiningDeathRules` (配 `MiningDeathRulesGameTests`) |
> | 矿洞进入收费 | `MiningServerConfig.ENTRY_FEE_*` + `entry.MiningWebUiActions` (三档默认值仍为 0, 即功能在但未开价) |
> | 矿洞维度禁止设置重生点 | `com.miningdim.rules.RulesSystem` |
> | 任务系统接入 WebUI 平板面板 | `quest.QuestWebUiActions` + `webui/src/pages/QuestsPage.tsx` |
