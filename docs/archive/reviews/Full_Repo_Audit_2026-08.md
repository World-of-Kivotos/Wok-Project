...
>
> - **冻结日期**: 2026-08-15
> - **冻结基线**: `49d5283` (WebUI 全量接线合并后)
> - **归档日期**: 2026-09-20 (归档基线 `bd0c4588`)
> - **现行真源**: 无单一接替者。本报告 112 条发现中, 六条 Critical 已全部闭合, 大量 Major/Minor 已修;
>   逐条现状请以代码为准, 勿照本文行号取证(实测 85 个位置路径中已有 8 个文件被删除)。
>
...
> **仍未闭合、已搬进现役文档的条目**:
>
> - F017 / F078 特勤悬赏系统整条未实现 -> `docs/SpecialAgent_Job_DesignSpec.md`
> - F068 婚姻"预约场地"未实现 -> `docs/Marriage_System_DesignSpec.md`
> - F047 护盾全伤害免疫窗定性, 复核降为 Minor 但**未结案, 待拍板** -> `docs/Armorer_Armor_System_DesignSpec.md`
>
> **本文中已作废、照做会造成损害的处置建议**:
>
> - 第三章 P0"摘掉 stacking 子系统装配": `StackingConfig.ENABLED` 已补上热重载总开关,
>   合并候选也已改成硬编码四种农场动物白名单。照本文注释掉装配行, 等于把一个已修好的子系统整体下线;
>   正确动作只是把 toml 里的 `enabled` 改成 false。
