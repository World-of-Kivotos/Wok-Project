...
> ## 归档件 — 历史记录, 不是实现依据
>
> - **冻结日期**: 2026-08-12 (六阶段全部完成, 815 个 GameTest 全绿)
> - **冻结基线**: `f7dbb73` (守恒律崩溃测试)
> - **归档日期**: 2026-09-20 (归档基线 `bd0c4588`)
> - **现行真源**: 代码 `com.miningdim.store` 与 `com.miningdim.persistence` 两包
>
...
> **第六节的遗留项处置**:
>
> - 第 2 条 补扣款失败的业务策略 (记欠账 / 收回资产 / 永久隔离三选一) —— **仍待拍板**, 已搬进经济总表
> - 第 3 条 运维口径 (数据库与存档同生共死、备份须含 `-wal`/`-shm`、真服复核 `PRAGMA synchronous`) —— **仍未写进部署文档**
> - 第 4 条 市场交付与背包非原子 —— **仍成立**, 已搬进经济总表
> - 第 6 条 旧库自动导入从未在真实存档上演练 —— **仍未做**
> - 第 7 条 `inprogressAssemblySurvivesSaveLoadRoundTripAndStillDelivers` 偶发失败 ——
>   **已于 2026-09-19 闭合**: 真因是 Forge 配置的 `autosave()` + `FileWatcher` 自触发异步重载,
>   修复见 `bd0c4588`(GameTest 期间摘掉配置热重载监视器), 连续五轮 `All 1474 required tests passed`。
