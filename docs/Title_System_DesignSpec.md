...
| 4 | 赞助专属称号违禁词表的初始内容 | TODO（服主补充到 `miningdim-title.toml` 的 `custom.bannedWords`，可热改；当前默认值即 13.3 列出的八个词） |

---

## 十三、赞助专属称号 (DECIDED，2026-09-26)

赞助玩家可以拥有**一个**专属称号，文字、外框、颜色都由玩家自己定。它和成就称号一样只是外观，不带任何属性。按 Mojang 商用准则，纯外观的赞助回馈是允许的。
...
| 自定义限制 | 按本章默认值：长度、字符集、违禁词、亮度下限、7 天冷却 |
...
| 违禁词 | 可配置列表，默认包括 管理、服主、官方、客服、OP、GM、admin、owner 等。匹配前先做归一化：转小写、全角转半角、去掉空格和符号，然后按子串匹配，防止"管 理""ＯＰ"这类绕过 |
...
- 阈值在服务端配置 `miningdim-title.toml` 的 `custom` 段：`maxLength`、`maxContentChars`、`allowedSymbols`、`bannedWords`、`minLuminance`、`customEditCooldownDays`。由 `TitleSystem` 走标准的 `registerConfig` 注册（GameTest 下由 `GameTestConfigWatchGuard` 统一摘掉监视器），每次校验实时读取，热改即生效。校验器 `CustomTitleValidator` 是纯函数，只认阈值快照 `CustomTitleRules`。
...
- 违禁词归一化：NFKC（全角转半角、兼容字转标准字）→ 转小写 → 只保留字母和数字。词表两边做同一套归一化后按子串匹配。代价是短英文词（`op`、`gm`）会顺带拦下含这些字母组合的单词（如 `shop`），配置注释里已写明，按需调整词表。
- 笔画分隔：有一类字本身算内容字符、看上去却只是一笔或一个记号——长音符 `ー`、单笔画汉字 `一丨丶丿乀乁乛亅` 与片假名 `ノ`、重复记号 `々〻ゝゞヽヾ`。它们照常可以写进称号，但夹在违禁词中间起的是分隔作用（`管ー理`、`服一主`、`OーP`），所以称号这一侧归一化后再去掉这些字比一次，任一次命中即算含违禁词。词表一侧不去掉：服主写进词表的词含这些字时（如 `一哥`）仍按原样匹配，不会被放宽成更短的词。ASCII 的 `1`、`l`、`I` 不在此列（去掉它们会误伤普通英文名），这类写法交给管理员的锁定与代设置处理。
...
- 实现口径：提交依次判断资格、锁定、冷却、校验，任何一步被拒都不写库、不消耗冷却；冷却中照样可以预览，回显里附带下次可修改时间。冷却起点就是 `updated_at`，即玩家本人最近一次成功修改的时间，管理员"清除冷却"把它置 0。管理员代设置既不开始也不清除玩家的冷却：沿用原记录的 `updated_at`，新建的记录记为 0，所以管理员预先备好的记录不会挡住玩家自己的第一次设置；锁定状态保持不变。要禁止玩家再改，用锁定（13.7）。
...
| `/mtitle custom preview <颜色> <粗体> <文字>` | 预览，不消耗冷却 |
| `/mtitle custom set <颜色> <粗体> <文字>` | 提交专属称号，受 7 天冷却限制 |
...
| `/mtitle custom admin set <玩家> <颜色> <粗体> <文字>` | 代为设置，**不受冷却限制**，也不改变玩家自己的冷却，但仍然过字符集和亮度校验。违禁词对管理员放行 |
