...
public record PlasmaShieldState(double shield,
    ...
    public static PlasmaShieldState full(PlasmaShieldConfig.Stats stats) {
        return new PlasmaShieldState(stats.capacity(), 0.0D, false, 0, 0);
    }
    ...
    public static PlasmaShieldState read(ItemStack stack, PlasmaShieldConfig.Stats stats) {
        ...
        return sanitize(new PlasmaShieldState(shield, heat, overheated, rechargeDelay, heatCoolDelay), stats);
    }
    ...
    /**
     * Absorption is bounded by both remaining energy and remaining thermal budget. This makes one combined hit
     * equivalent to the same hit split into several TaCZ damage segments.
     */
    public static HitResult absorb(PlasmaShieldState input,
        ...
        if (state.overheated || state.shield <= EPSILON) {
            PlasmaShieldState delayed = new PlasmaShieldState(
                    state.shield, state.heat, state.overheated, rechargeDelay, heatCoolDelay);
        ...
        double absorbed = Math.min(incomingDamage, Math.min(state.shield, thermalAllowance));
        ...
        PlasmaShieldState next = new PlasmaShieldState(
                nextShield, nextHeat, nextOverheated, rechargeDelay, heatCoolDelay);
    ...
    public static PlasmaShieldState tick(PlasmaShieldState input,
        ...
        for (int tick = 0; tick < elapsedTicks; tick++) {
            ...
            if (rechargeReady) {
                shield = Math.min(stats.capacity(), shield + rechargePerTick);
            }
        }
        return sanitize(new PlasmaShieldState(shield, heat, overheated, rechargeDelay, heatCoolDelay), stats);
    }
    ...
    public static PlasmaShieldState sanitize(PlasmaShieldState state, PlasmaShieldConfig.Stats stats) {
        ...
        return new PlasmaShieldState(shield, heat, overheated, rechargeDelay, heatCoolDelay);
    }
