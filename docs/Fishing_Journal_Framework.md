...
当前阶段仅提供鱼种目录、查阅界面和收藏进度。渔夫经验、职业能力、委托、厨师/农夫/酿酒师配方由后续功能接入；收藏记录不冒充亲手钓获，也不发放经验。
...
默认目录包括 4 种原版鱼和 66 种 Tide 1.6.5 自有鱼种。Tide 元数据引用其现有描述、地点、条件翻译键及物品模型，不复制其代码、贴图或描述正文。缺少 Tide 时只加载原版目录；Tide 版本不等于 1.6.5 时跳过该兼容目录并记录版本提示。
