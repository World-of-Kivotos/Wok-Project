...
/*
 * 本项目对上游的三处签名放宽 (本文件另两处见 Toasts / AnchoredToasts)。
 *
 * 原因: tsconfig 开了 exactOptionalPropertyTypes, 该档下 `type?: string` 表示"这个键要么不存在,
 * 要么是 string", 显式传 undefined 不合法; 而实参 ToastObject 的 type 是 `string | undefined`。
 * 上游按默认档写, 到本工程就报 TS2379/TS2375。
 *
 * 补 `| undefined` 是语义上更准确的写法, 不是妥协 —— 这几个位置本来就接受 undefined。
 * 重新 `shadcn add @coss/ui` 覆盖本文件后需要照此重打一遍, 搜索 exactOptionalPropertyTypes 即可定位。
 */
...
function Toasts({
  ...
  // exactOptionalPropertyTypes 放宽, 见 upsertReplayClassName 上方说明。
