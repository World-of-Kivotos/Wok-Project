 ...
 * RNG / 生产者 UUID, 计算: 消耗多少矿 + 产出几块板 (含品质 +1、闪耀概率) + 残骸返还 (闪耀失败)。
 *
 * 矿石消耗与产出为定值/概率 (3.2): 极品 1 锭 -> 2 板; 闪耀 2 锭 -> 概率 1 板 (失败返还残骸); 其余 1 板。
...
public final class NanoProduction {
    ...
    /** 生产结算结果 (BE 据此扣矿、产板、退残骸; 不可变)。 */
    public record Result(int oreConsumed, int platesProduced, int debrisRefund) {
    }
