...
1. **同域内的残留敞口**: 业务域隔离堵住了跨业务复用, 但同域内仍有一条窄路径 —— 账本有记录、SQLite 无
   对应行、且金额恰好等于开箱价时, `charge` 的全元组校验会通过并返回已有状态, `EconomyCaseOperations`
   把 `COMPLETED` 映射为 `true`, 仍会跳过扣款。前置条件是运维单独回滚 `miningdim_cases.db` 而保留世界
   NBT。彻底消除需改用服务端派生幂等键。阶段 5 单事务化后此路径自然消失 (钱与资产同库同事务, 不可能
   出现"账本有、资产库无")。
2. **补扣款失败的业务策略**: 玩家崩溃后把回滚回来的钱花光, 恢复时扣不到款。三选一: 记欠账 / 收回资产 /
   永久隔离。属产品决策, 未定。
3. **运维口径**: 即使代码修好, 也应把"数据库文件与世界存档必须同生共死, 禁止单独回滚其一"写进部署文档。
   合库后风险面收敛为单一文件, 但备份必须覆盖 `.db` 与 WAL 附属文件。
4. **market 与玩家背包之间仍非原子**: 迁移解决的是"钱与数据库资产", 但市场交货写的是玩家背包 NBT,
   那是第三个存储。仍需持久 outbox 或邮箱式领取, 不要误以为合库后市场就完全原子了。
5. **`docs/WebUI_Architecture_DesignSpec.md:124`** 把该窗口描述为"极小", 与实测的 5 分钟不符;
   同文档 `:135` 声称市场买卖在单事务内完成, 与实现不符 (实现方已在 `MarketEngine` 注释中记录该偏离)。
   完成迁移后应一并回写这两处。
