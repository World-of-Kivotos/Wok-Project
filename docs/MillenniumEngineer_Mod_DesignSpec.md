# 千年工程师 职业 Mod — 设计规格文档

## 文档元信息

- 用途: 千年工程师职业实现阶段的唯一架构、数值与机制参考。所有常量以本文档为准, 不得凭记忆改写。
- 目标平台: Minecraft 1.20.1 + Forge 47.x + Java 17。所有 API 名称以此版本为准, 不得套用其他版本语法 (与 MiningDimension 规格第一章同源约束)。
- 部署环境 (硬约束, 影响全部战斗向数值): 目标公服人均初始最大血量 80 (原版 20 的 4 倍); 装有 TACZ 枪械 mod, DPS 远高于原版, 全自动每秒多次命中、狙/霰弹单发可超 80 且常带穿甲, 击杀时间以秒计; 死亡不掉落。一切战斗向数值一律用 "最大血量百分比 / 拦截致死" 建模, 严禁套用原版 20 血常量。
- 状态图例: DECIDED 已定稿 / PENDING 待拍板 / TODO 实现期补全。
- 数值状态: 等级曲线沿用农业职业已交叉验证曲线; 每日衰减曲线与达标天数已实算验证 (见第八章)。
- 阻塞项: 进入编码前须先解决第十二章列出的全部 PENDING。

---

## 一、职业定位与设计目标 (DECIDED)

1. 定位: 仿 FF14 生产职业。玩家无需专职, 可同时持有并体验全部职业; 工程师只是其一。
2. 核心卖点: 在工程师工作台用 "纳米护甲板" 修复一切原本无法维修的护甲 (公服存在大量无原材料修复路径的模组护甲)。
3. 经济定位: 纳米修复 = 贵但万能。即使最低级护甲板, 一个铁锭也做不出 (成本高于铁砧+铁锭), 修复效果也不如铁砧; 唯一优势是 "能修一切"。纳米修复不与原版铁砧/原材料修复抢常规护甲的生意, 只补 "原版修不了" 的空缺。
4. 特效定位: 修复护甲时掷出的 **一次性副产品**, 不是永久词条; 再次用纳米护甲板维修会丢弃全部旧特效。永久词条系统留给未来其他职业, 本职业不做永久词条。
5. 等级范围: 1 - 10, L10 毕业。正常玩家约一个月毕业; 肝满党有上限地更快 (约 16 天, 见第八章)。
6. 反代练: 经验与护甲板生产强绑定生产者本人 (谁产谁得), 配合工作台上锁 (第九章)。

设计三闸门 (与农业职业同构): 每日有效经验软上限 (日产出天花板) × 分级经验需求 (总量与后期坡度) × 等级门控解锁的护甲板档位/生产台档位。

---

## 二、核心机制总览 (DECIDED)

| 机制 | 一句话 | 章节 |
| --- | --- | --- |
| 纳米护甲板 | 矿石一段式直产的分档修复消耗品 (低/中/高/极品/超凡 + 闪耀) | 三 |
| 生产台 | 投矿 + 选档的处理台, 生产需主动交互 (纳米校准小游戏), 挂机无产出 | 四 |
| 修复 | 连续效率曲线: 低档小固定值 -> 高档按最大耐久百分比 -> 闪耀 100% | 五 |
| 特效 | 高级板起掷出的一次性副产品, 按件生效, 再次维修清空 | 六 |
| 等级 | 1-10, 独立经验池, 每日软上限+衰减, 谁产谁得 | 七、八 |
| 上锁 | 工作台 owner 锁 + 护甲板 producer 盖章, 双层反代练 | 九 |

---

## 三、纳米护甲板与生产模型 (DECIDED)

### 3.1 护甲板档位

六档物品 (DeferredRegister Item): 低级 / 中级 / 高级 / 极品 / 超凡 / 闪耀。建议统一用一个 `enum NanoTier` 承载 5 档 + 闪耀特例, 供生产/修复/特效逻辑共用, 避免散落比较。

### 3.2 一段式生产 + 矿石绑档 (DECIDED)

矿石直接生成护甲板 (无中间材料)。低矿造不了高板: 选中目标档必须 <= 输入矿石所允许的最高档。

| 输入矿石 | 可造最高护甲板档 |
| --- | --- |
| 铁锭 | 低级 |
| 金锭 | 中级 |
| 钻石 | 高级 |
| 下界合金锭 | 极品 |
| 下界合金锭 + 超凡生产台 + L9 | 超凡 |
| 下界合金锭 + 闪耀生产台 + L10 | 闪耀 (概率产出) |

高档矿可降级造低板 (浪费); 低档矿永远造不了高板。每档单板的矿石消耗量、生成耗时进 config。

---

## 四、生产台与主动交互 (DECIDED)

### 4.1 选择式处理台 GUI

生产台不是熔炉式自动转化, 而是 "投矿 + 选档" 处理台:

- 输入矿石槽; 一排护甲板档位选择按钮; 进度条; 输出槽 (仅主人手取, 不对漏斗暴露); 锁状态指示。
- 选档按钮可点 = 同时满足三道门, 否则置灰 + tooltip 说明原因。

三道门 (选档能否点亮):

| 门 | 规则 |
| --- | --- |
| 矿石档位 | 3.2 表, 低矿造不了高板 |
| 工程师等级 | 等级未解锁该档则置灰 (解锁表见 7.2) |
| 生产台档位 | 机器档不够则置灰; 机器档同时决定生成耗时 |

服务端权威: 客户端置灰仅为提示, 服务端在消耗矿石/产出前重新校验三道门 + 锁主人, 绝不信客户端选择 (C5)。选档走原版 `AbstractContainerMenu.clickMenuButton(tierIndex)`, 不新开网络包。

### 4.2 纳米校准 (主动生产, 反挂机) (DECIDED 主方案)

进度不随时间自动涨, 需主动交互:

- GUI 内一条扫描条, 游标左右扫, 每轮随机出现一个绿色 "校准区"。
- 游标进绿区时点击: 命中 -> 进度推进 + 品质条++; 未中 -> 进度只推一点点 + 品质不涨。
- 不点 = 进度不动 -> 挂机无产出。绿区位置每轮随机 -> 防固定连点器。

品质条结算时影响 (FF14 味, 与已有系统咬合):

| 品质 | 影响 |
| --- | --- |
| 产量 | 高品质有概率额外 +1 护甲板 |
| 特效 | 高级板起, 品质越高修甲时掷出特效概率越高 |
| 经验 | 品质越高该次生产的原始经验越高 (再过每日衰减档入账) |

服务端权威: 扫描条时序、绿区位置、命中判定全在服务端; 客户端只渲染 + 回传点击 tick, 服务端判窗口内才算命中。此为全工程客户端最重的一处 (实时条渲染 + 时序容差), 比普通进度条费工但有界可做。

降级方案 (PENDING 取舍): 去扫描条, 改 "每隔几秒亮一次校准按钮, 点中即推进" (无技巧、防宏弱、实现简单), 后续再升级。默认主方案 (扫描条)。

---

## 五、修复机制与经济定位 (DECIDED)

### 5.1 连续修复效率曲线

取消 "修复次数上限" 机制 (原构想的 "低级有次数上限、中级解除" 是伪闸门), 改为连续效率曲线。低档固定值利好低耐久护甲, 高档百分比利好高耐久/模组护甲, 形成用途分工。数值为示例, 真值进 config。

| 护甲板档 | 修复模型 (示例) | 适合护甲 | 特效 |
| --- | --- | --- | --- |
| 低级 | 极小固定值 (+5 耐久/板) | 入门 "能修", 远低于铁砧 | 无 |
| 中级 | 小固定值 (+25) | 低耐久护甲, 仍低效 | 无 |
| 高级 | 较大固定值 (+80) | 中耐久护甲 | 开始可能掷特效 |
| 极品 | 按最大耐久百分比 (25%) | 高耐久/模组护甲 | 可能掷特效 |
| 超凡 | 更高百分比 (60%) | 高耐久/模组护甲 | 可能掷特效 |
| 闪耀 | 100% + 清旧特效 + 必定新特效 | 任意 | 必出 |

### 5.2 与原版修复的关系

- 禁用经验修补 (Mending): 防止原版回耐久偷偷越过特效的耐久门槛。**范围 PENDING**: 全服禁用 (影响所有人所有物品) vs 仅在带纳米特效的护甲上禁用。
- 铁砧旁路: 带特效护甲多为 "原版修不了才用纳米修" 的那类, 铁砧本就修不了, 旁路在实战里多不成立; 仅 "本可修的甲特意纳米修来拿特效" 才碰得到。**建议不拦** (省一层 `AnvilUpdateEvent`), 让铁砧修这类甲保留特效。PENDING 你拍板。

### 5.3 可实现性

直接改 `ItemStack.setDamageValue / getDamageValue` 对任意 `Damageable` 物品 (含模组护甲、无原版修复配方者) 均生效, 这是 "修一切" 的技术基础; Unbreakable NBT 物品的处理策略实现期定 (TODO)。

---

## 六、特效系统 (DECIDED)

### 6.1 基本规则

- 特效是修复掷出的一次性副产品, 非永久词条; 完全独立于原版附魔系统 (存自定义 NBT tag, 不写 Enchantments)。
- 高级板起可能掷特效, 闪耀必定。再次用纳米护甲板维修 -> 丢弃全部旧特效。闪耀 = 修复 + 清旧特效 + 必出新特效。
- **特效按件生效** (非整套)。平衡红线是 "是否打破枪战 attrition", 不是 "是否打得过小怪" (见部署环境)。

### 6.2 四个特效 (数值为示例, 进 config)

| 特效 | 机制 | 失效条件 | 按件叠加处理 |
| --- | --- | --- | --- |
| 纳米重塑 | 缓慢回护甲自身耐久 | 损失 > 25% 耐久 (阈值可配, 建议可放宽到 40%, 否则枪火下立即触发) | 按件 (耐久逐件, 不汇聚), 安全 |
| 纳米机能修复 | 缓慢回穿戴者血量 (按最大血量百分比, 单件如 2%/s) | 护甲耐久 < 50% | 按件 + 递减安全阀: 100% / 50% / 25% / 12.5% (四件合计约 1.875 倍, 非 4 倍), 防 PvP 间隙滚雪球 |
| 纳米多重护盾 | 触发后 X 秒 (可配) 免疫全部伤害 (重定义: 枪环境下 "挡单次伤害" 对全自动无用); 每 60 秒生成一次, 生成 5 次后消除 | 5 次用尽 | 按件 (各自 5 次), 有界 |
| 纳米末影心肺反应器 | 拦截本将致死的一击 (非血量阈值, 防高爆发 overshoot), 复活到 % 最大血量 (如 50% = 40 血) + 短伤害免疫窗 | CD 中 | **共享 CD** (人级, 30 min); 耐久代价 = 每件带此效果的甲各扣 % 最大耐久; 叠穿 = 冗余保险 (一件碎了有备份) 非额外命数 |

设计原则: 作用于 "护甲自身" 的效果 (重塑、护盾) 按件生效安全; 作用于 "玩家本体" 的效果 (机能修复、图腾) 按件会线性放大成无敌/多命, 故机能修复加递减、图腾共享 CD。

### 6.3 状态存放

- 特效标记 (哪几件带何效果): 逐件存 ItemStack NBT。
- 图腾共享 CD (`nanoReactorCdEndTick`): 人级, 存玩家 capability (见第十章)。
- 护盾剩余次数 / 60s 计时 / 重塑·机能修复的 tick: 逐件存 ItemStack NBT, 由 `TickEvent.PlayerTickEvent` 遍历护甲槽读取。
- 仅穿戴中的护甲参与 tick 类效果 (背包/箱子内不 tick), 实现期明确 (TODO)。

### 6.4 客户端表现 (DECIDED 第一批 / 第二批)

- 第一批 (便宜, 与数值解耦): 服务端 `ServerLevel.sendParticles` 自动广播粒子 (重塑青色火花 / 机能修复绿色治疗粒子 / 护盾蓝色六边环 / 图腾紫色末影粒子) + 图腾触发用原版 `broadcastEntityEvent((byte)35)` 全屏动画, 挂在 "拦截致死" 那一刻。
- 第二批: 自定义 HUD 叠层 (护盾剩余层数 / 图腾 CD 倒计时 / 生效特效图标), 需 S2C 状态包 + 客户端 `RenderGuiOverlayEvent`。依赖 6.3 状态模型, 第二批做。
- 同步坑: 图腾扣耐久后须调 `player.containerMenu.broadcastChanges()`, 否则客户端耐久条不刷新。

---

## 七、等级与经验系统 (DECIDED)

### 7.1 等级需求表 (沿用农业职业已验证曲线, 总 61,900)

| 等级 | 升至下一级所需经验 | 累计 (达到本级) |
| --- | --- | --- |
| 1 | 3,300 | 0 |
| 2 | 3,800 | 3,300 |
| 3 | 4,500 | 7,100 |
| 4 | 5,300 | 11,600 |
| 5 | 6,300 | 16,900 |
| 6 | 7,400 | 23,200 |
| 7 | 8,800 | 30,600 |
| 8 | 10,300 | 39,400 |
| 9 | 12,200 | 49,700 |
| 10 | (毕业) | 61,900 |

### 7.2 等级解锁表 (每两级解锁一档, 与农业同构)

| 等级 | 解锁护甲板档 + 生产台档 |
| --- | --- |
| L1 | 低级板 + 低级生产台 |
| L3 | 中级板 + 中级生产台 |
| L5 | 高级板 + 高级生产台 (特效从此开始) |
| L7 | 极品板 + 极品生产台 |
| L9 | 超凡板 + 超凡生产台 |
| L10 | 闪耀板 + 闪耀生产台 |

### 7.3 每日经验软上限与衰减 (DECIDED 主方案)

按 "当日已结算有效经验" 判定档位, 对新产生的原始经验乘系数。尾档不归零 (区别于农业公服 x0 方案), 以奖励肝满党 (见第八章)。

| 当日有效经验区间 | 系数 |
| --- | --- |
| 0 - 2000 | x1.00 |
| 2000 - 2800 | x0.40 |
| 2800 - 3400 | x0.20 |
| 3400 - 3800 | x0.08 |
| 3800 以上 | x0.02 (涓流, 实际软顶约 4000/天) |

衰减曲线陡峭度 = "肝度奖励" 主旋钮, 全进 config。

### 7.4 经验来源 (谁产谁得, 反代练) (DECIDED)

两个结算点产出原始经验, 经 7.3 衰减档乘系数入账:

| 来源 | 给谁 | 判定 |
| --- | --- | --- |
| 生产护甲板 | 生产者本人, 取出时结算 | 取出者 UUID == 板子 `producerUUID`, 且 `productionXpPending == true` (取走即清, 防塞回再取重复刷) |
| 用护甲板修甲 | 板子生产者本人 | 消耗板子 `producerUUID` == 修复者 UUID; 不匹配照样能修, 但无经验 |

单档原始经验 (示例, 进 config): 低级 15 / 中级 30 / 高级 60 / 极品 110 / 超凡 200; 用自己板修甲额外 +50%。真正的天花板是 7.3 每日软上限, 单板经验只决定 "今天要产多少块打满全速段"。

---

## 八、达标天数推演验证 (DECIDED, 已实算)

以 "日投入原始经验" 为画像, 经 7.3 衰减档换算当日实际入账有效经验, 再以总需求 61,900 折算天数。

| 画像 | 日投原始经验 | 当日实际入账 (有效) | 满级天数 |
| --- | --- | --- | --- |
| 休闲 (打到舒适即下) | 2,200 | 2,080 | 29.8 |
| 正常 (每天数小时) | 4,000 | 2,800 | 22.1 |
| 肝 (大半天) | 7,000 | 3,400 | 18.2 |
| 肝满 (全天) | 12,000 | 3,800 | 16.3 |
| 极限 (挂满 + 巨量矿) | 22,000 | 4,000 | 15.5 |

结论: 休闲党约一个月; 肝满党有上限地压到约 16 天; 软底约 15.5 天, 再挂也快不过半个月 (杜绝一天毕业)。休闲到肝满约 2 倍差距, 肝得多确实快但边际递减。"日投原始经验" 上限由主动校准 (4.2) + 护甲板档 + 生产台速度 + 矿石量共同决定, 实现期标定其与在线时长的换算。

---

## 九、工作台上锁与反代练 (DECIDED)

### 9.1 上锁

- BE 存 `ownerUUID` (`Block.setPlacedBy` 记录放置者)。
- 潜行右键空手切换 锁/不锁 (仅主人); 普通右键开 GUI。锁状态用 actionbar + 粒子反馈。锁切换走 `clickMenuButton` 或轻量 C2S, 不新开网络包。
- 锁定时: 非主人 (OP / 信任名单除外) 拒绝开 GUI / 取物。
- 输出槽不对漏斗暴露 (`getCapability(ITEM_HANDLER, side)` 只暴露输入或输出只读包装): 挡自动化 farm + 保证 "必须人手取" 这条结算前提。

### 9.2 双层归属

- 锁 = 访问控制 (别人别乱用)。
- `producerUUID` = 经验归属 (经验只给生产者)。即使将来解锁共享机器, 生产经验仍按 `取出者 == producerUUID` 给, 不漏经验。

### 9.3 反代练覆盖

| 刷取手段 | 是否被挡 | 机制 |
| --- | --- | --- |
| 买板子升级 | 挡 | 生产经验只给 "自己取", 修复经验只给 producerUUID 匹配 |
| 小号产板喂大号 | 挡 | UUID 不匹配, 两种经验都拿不到 |
| 漏斗挂机自动产 | 挡 | 输出不对漏斗暴露 + 必须手取结算 + 4.2 校准需主动 |
| 取出放回反复刷生产经验 | 挡 | `productionXpPending` 首次取走即清 |
| 囤矿一次性梭哈秒升级 | 挡 | 7.3 每日软上限封住单日入账 |

---

## 十、架构落地 (DECIDED)

### 10.1 子系统接线

- 新建 `com.miningdim.engineer` 子系统, 提供一个 `EngineerSystem implements Subsystem`, 在 `register` 内完成自己的 DeferredRegister (Block/Item/BlockEntity/MenuType) + 事件订阅 + 服务注册。
- 在 `MiningDim.registerSubsystems()` 追加一行 `new EngineerSystem()`。跨子系统协作只经 core 门面 + MiningServices, 不硬 import 他系统实现类 (铁律 2/3)。

### 10.2 玩家职业数据 (关键裁决: 不新挂 capability)

- 工程师等级数据 (`engineerLevel` / `engineerXp` / `dailyEngineerXp` + 翻日戳 / `nanoReactorCdEndTick`) 作为字段并入 `entry.MiningPlayerData` (`IMiningPlayerData` 接口扩方法), 复用其已有的 attach / `PlayerEvent.Clone` (reviveCaps/invalidateCaps) / serialize / copyFrom 全套管线。
- 严禁照搬农业 spec 字面新建独立 capability + 新 `AttachCapabilitiesEvent<Entity>`: 工程已有 "双 capability 重复 attach -> 双重传送/双重引用计数" 既裁隐患 (entry 为唯一权威), 第三套 Provider 会放大隐患。
- 多职业方向 (FF14 已确认 N >> 1): 玩家职业数据应朝 "一个 capability 持 `Map<JobId, JobProgress>`" 演进 (一次到位成多职业形状), 重的数据驱动框架 (等级曲线/每日衰减表) 可增量补。
- `deserializeNBT` 对旧存档缺新键给默认值 (level=1, xp=0, cd=0); 扩 `IMiningPlayerData` 后多维 grep 找全实现/mock 补新方法。

### 10.3 数值配置 (C6 硬约束)

全部平衡数值 (等级曲线 / 每日衰减档 / 单板经验 / 矿石绑档 / 各档生成耗时 / 修复曲线 / 特效阈值与系数 / 图腾 CD 与复活百分比 / 护盾免疫窗与次数 / 机能修复递减系数 / 闪耀概率) 进 `MiningServerConfig` 的 `ForgeConfigSpec`。BlockEntity / 业务类内出现硬编码字面量即缺陷。config 实时 get 不缓存; 进行中任务的耗时建议起算时快照。

### 10.4 BE 无玩家上下文

BlockEntity tick 内无玩家引用。读玩家等级的判定收敛到有玩家在场的交互帧 (Block.use 打开 GUI / 取物 / 选档), 由 ServerPlayer 经 entry capability 读取; 多人共用一台机器时 "谁的等级算数" 默认按主人 (`ownerUUID`), 主人离线策略 (暂停高档 / 按记录等级) 实现期拍板 (TODO)。

### 10.5 容器 GUI 基础设施 (从零搭)

工程现状零 menu/screen 基础设施。需新建: `DeferredRegister<MenuType<?>>` + `IForgeMenuType.create((id, inv, buf) -> ...)` (用 `buf.writeBlockPos` 传坐标); `AbstractContainerMenu` (必须正确实现 `quickMoveStack` 防 Shift 点击吞物/死循环, 与 `stillValid`); 进度走 `ContainerData` (int-only, 自动同步给开 GUI 者); BE `implements MenuProvider` + `NetworkHooks.openScreen`; `AbstractContainerScreen`; 客户端 `FMLClientSetupEvent.enqueueWork(() -> MenuScreens.register(...))`; gui png + 槽位坐标 + lang。注意全为 1.20.1 写法, 严禁套 1.20.4+ custom payload / 1.20.5+ MapCodec。

---

## 十一、可实现性结论 (Forge 1.20.1, DECIDED)

全部机制在 Forge 1.20.1 可实现, 无不可实现项。工作量与坑分布:

| 模块 | 可实现性 | 关键 API | 工作量 |
| --- | --- | --- | --- |
| 六种护甲板物品 | 可实现 | DeferredRegister Item | 小 |
| 等级/经验/CD 数据 | 可实现 | 并入 entry.MiningPlayerData (NBT) | 小 |
| 修复 (改耐久) | 可实现 | ItemStack.setDamageValue 对任意 Damageable | 小 |
| 特效 tick (重塑/机能修复) | 可实现 | PlayerTickEvent 遍历护甲槽 + 自定义 NBT | 中 |
| 多重护盾 (免疫窗) | 可实现 | LivingAttackEvent/LivingHurtEvent + NBT 计时 | 中 |
| 图腾 (拦截致死) | 可实现 | LivingDeathEvent 取消 + 复刻 totem 行为 + 人级 CD | 中 |
| 生产台五档/选档/绑档 | 可实现 | BE ticker + clickMenuButton + config | 中 |
| 容器 GUI (三台) | 可实现但有坑 | 见 10.5, 从零搭一套 | 大 |
| 纳米校准 QTE | 可实现但有坑 | 服务端时序权威 + 客户端实时渲染 | 大 (客户端最重) |
| 上锁 + producer 盖章 | 可实现 | ownerUUID/producerUUID NBT + 输出不暴露漏斗 | 中 |

---

## 十二、待确认实现项 — 进入 Code 前须拍板 (PENDING)

1. 禁用经验修补范围: 全服禁用 vs 仅纳米特效甲禁用。
2. 主动交互取舍: 扫描条 QTE (默认主方案) vs 降级 "亮按钮点击"。
3. 铁砧修纳米特效甲是否保留特效 (建议不拦)。
4. 闪耀: 单次成本 c + 出货概率 p + 是否保底 (N 次必出)。
5. 多人共用机器主人离线时的产出策略 (10.4)。
6. Unbreakable NBT 护甲的修复处理 (5.3)。
7. 实现期标定的 config 真值: 各档生成耗时 / 修复固定值与百分比 / 特效阈值与系数 / 图腾 CD 与复活百分比 / 护盾免疫窗秒数 / 单板经验 / 矿石绑档消耗量 / "日投原始经验" 与在线时长换算。

---

## 十三、实现期工作分解 (确认上述 PENDING 后展开)

按单文件原子提交、每模块配 TDD 断言具体数值与边界推进。

第一批 (零风险, 与交互/数值解耦, 可独立跑通):
1. `entry.MiningPlayerData` 扩字段 (`engineerLevel/engineerXp/dailyEngineerXp+翻日戳/nanoReactorCdEndTick`) + `IMiningPlayerData` 扩接口 + serialize/deserialize/copyFrom + 全实现/mock 补齐。
2. `EngineerSystem` 子系统骨架 + 接进 `MiningDim.registerSubsystems()` 一行。
3. 服务端粒子特效 + 图腾 "拦截致死" 动画钩子 (LivingDeathEvent)。
4. `MiningServerConfig` 加全部数值 spec (第七、八章曲线 + 第五、六章示例值 + 第三章绑档)。

第二批 (依赖第一批 + PENDING):
5. 六种护甲板物品 + `NanoTier` 枚举。
6. 三种工作台方块 + BlockEntity + 容器 GUI 基础设施 (10.5)。
7. 投矿选档 + 三道门校验 + 矿石绑档 + 生成耗时。
8. 上锁 (ownerUUID + 潜行切换 + 输出防漏斗) + producer 盖章 + 两个经验结算点 + 每日衰减入账。
9. 修复曲线 + 特效掷出/清空 + 四个特效完整逻辑 (含机能修复递减、图腾共享 CD、护盾免疫窗)。

第三批:
10. 纳米校准 QTE (服务端时序 + 客户端渲染) + 品质条结算。
11. 自定义 HUD 叠层 (护盾层数 / 图腾 CD / 特效图标) + S2C 状态包。

测试断言示例: 总需求 61,900; 日投 12,000 原始经验入账 3,800 有效, 满级 16.3 天; 铁锭只能选到低级板, 选高级板被服务端拒; 小号产的板给大号修甲不结算修复经验; 图腾 30min 内仅触发一次 (人级 CD)。
