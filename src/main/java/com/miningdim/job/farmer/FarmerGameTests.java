...
public final class FarmerGameTests {
    ...
    @GameTest(templateNamespace = MiningConstants.MODID, template = EMPTY, batch = BATCH)
    public static void onFarmlandBrokenRecyclesPlacementCount(GameTestHelper helper) {
        ServerPlayer player = MockGameTestPlayers.makeMockServerPlayerWithChannel(helper);
        FarmerSystem sys = new FarmerSystem();
        FarmerSavedData data = FarmerSavedData.get(player.server.overworld());
        int before = data.placedCount(player.getUUID());
        data.increment(player.getUUID());
        data.increment(player.getUUID()); // 模拟已放 2 块 (聚焦破坏回收本体, 取增量防跨测试串扰)。
        helper.assertTrue(data.placedCount(player.getUUID()) - before == 2, "precondition: 2 farmland placed");

        BlockPos abs = helper.absolutePos(new BlockPos(0, 1, 0));
        BlockState modFarmland = FarmerBlocks.farmland(FarmerTier.LOW).get().defaultBlockState();

        // 破坏 mod 耕地 -> 计数 -1。
        sys.onFarmlandBroken(new BlockEvent.BreakEvent(helper.getLevel(), abs, modFarmland, player));
        helper.assertTrue(data.placedCount(player.getUUID()) - before == 1, "breaking mod farmland recycles one placement");

        // 破坏原版泥土 -> 计数不变 (instanceof 守卫只认 FarmerFarmlandBlock)。
        sys.onFarmlandBroken(new BlockEvent.BreakEvent(
                helper.getLevel(), abs, Blocks.DIRT.defaultBlockState(), player));
        helper.assertTrue(data.placedCount(player.getUUID()) - before == 1, "breaking vanilla dirt does not recycle count");

        // 取消的破坏 -> 计数不变。
        BlockEvent.BreakEvent canceled = new BlockEvent.BreakEvent(helper.getLevel(), abs, modFarmland, player);
        canceled.setCanceled(true);
        sys.onFarmlandBroken(canceled);
        helper.assertTrue(data.placedCount(player.getUUID()) - before == 1, "canceled break does not recycle count");
        helper.succeed();
    }
